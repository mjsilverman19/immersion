...
import type { GoogleTextSearchResponse } from "@/lib/types/google-places";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const googleData: GoogleTextSearchResponse = await googleRes.json();

    const googlePlaces = (googleData.results || []).slice(0, 5).map((r) => ({
    ...
    const newGooglePlaces = googlePlaces.filter((g) => !existingGoogleIds.has(g.google_place_id));

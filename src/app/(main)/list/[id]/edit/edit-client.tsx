...
export default function EditListClient({ list, items: initialItems }: EditListClientProps) {
  const [title, setTitle] = useState(list.title);
  const [description, setDescription] = useState(list.description || "");
  const [cityId, setCityId] = useState(list.city_id || "");
  ...
  return (
    <div className="bg-cream min-h-screen p-4">
      <h1 className="mb-6 font-serif text-2xl text-ink">Edit List</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {items.length > 0 && (
          <div>
            <label className="mb-2 block text-sm font-medium text-ink">Places ({items.length})</label>
                ...
                <div key={item.place.id} className="rounded-xl bg-white p-3 shadow-sm">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-1">
                      <button type="button" onClick={() => moveItem(idx, -1)} className="rounded p-1 text-ink-light hover:bg-cream-dark" disabled={idx === 0}>↑</button>
                      <button type="button" onClick={() => moveItem(idx, 1)} className="rounded p-1 text-ink-light hover:bg-cream-dark" disabled={idx === items.length - 1}>↓</button>
                      <button type="button" onClick={() => removeItem(idx)} className="rounded p-1 text-rust hover:bg-rust-light/20">✕</button>
                    </div>
                  </div>
                  <input
                    type="text"

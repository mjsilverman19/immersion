...
export default async function FollowingPage({ params }: Props) {
  ...
  const { data: follows } = await supabase
    .from("follows")
    .select(
      "following:profiles!follows_following_id_fkey(id, username, display_name, avatar_url, home_city_id, city:cities!profiles_home_city_id_fkey(name))"
    )
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="px-4 py-2">
        {following.length === 0 ? (
          <p className="py-8 text-center text-sm text-ink-light">Not following anyone yet</p>
        ) : (

...
export function TypicalTimeControl({ day, hour, onDayChange, onHourChange }: TypicalTimeControlProps) {
  return (
    <div className="brand-surface w-full max-w-md rounded-2xl p-3">
      <div className="flex items-center gap-2 type-panel">
      ...
      <div className="mt-2.5 flex gap-1 overflow-x-auto scrollbar-hide" aria-label="Typical weekday">
        {WEEKDAYS.map((weekday) => (
          <button key={weekday.key} type="button" onClick={() => onDayChange(weekday.key)} aria-pressed={day === weekday.key} className={`min-h-9 min-w-10 flex-1 rounded-xl px-2 py-1.5 ${day === weekday.key ? "type-button bg-primary text-primary-foreground shadow-sm" : "type-body bg-background/40 text-muted-foreground hover:bg-background/70 hover:text-foreground"}`}>
            {weekday.short}
          </button>
        ))}
      </div>
      <div className="mt-2 flex items-center gap-3">
        <span className="type-body text-muted-foreground">12 a.m.</span>
        ...
        <span className="type-body text-muted-foreground">11 p.m.</span>
      </div>
    </div>
  );
}

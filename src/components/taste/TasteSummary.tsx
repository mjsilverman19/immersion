...
export function TasteSummary({ profile, topArea, onClose, onAdjust }: { profile: TasteProfile; topArea?: SelectedArea; onClose: () => void; onAdjust: () => void }) {
  return <div className="absolute left-1/2 top-24 z-50 w-[min(92vw,360px)] -translate-x-1/2 rounded-2xl border border-border bg-background/96 p-4 shadow-xl backdrop-blur">
    ...
    <div className="mt-4 flex gap-2"><button type="button" onClick={onClose} className="rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground">Explore my map</button><button type="button" onClick={onAdjust} className="rounded-full border border-border px-4 py-2 text-xs">Adjust</button></div>
  </div>;
}

...
export function AreaSheet({ selected, day, hour, onClose, onSelectVenue }: { selected: SelectedArea | null; day: WeekdayKey; hour: number; onClose: () => void; onSelectVenue: (venue: VenueRecord) => void }) {
  ...
  if (minimized) return <aside className="brand-surface absolute bottom-[172px] left-1/2 z-40 flex max-w-[92vw] -translate-x-1/2 items-center gap-2 rounded-full p-1.5 pl-4 md:bottom-5 md:left-5 md:translate-x-0">
  ...
  return (
    <aside className="brand-sheet absolute inset-x-2 bottom-2 z-40 mx-auto max-h-[calc(100dvh-7.5rem)] max-w-lg overflow-y-auto rounded-3xl p-5 md:inset-x-auto md:bottom-5 md:left-5 md:max-h-[72dvh] md:w-[390px]">
      ...
      <div className="mt-4 rounded-2xl border border-white/50 bg-background/40 p-4">
      ...
      <p className="mt-3 flex items-center gap-2 rounded-xl border border-white/50 bg-background/40 px-3 py-2 text-[11px] text-muted-foreground"><Route className="h-4 w-4 shrink-0" /> Deeper neutral shading and streets indicate a stronger modeled typical activity pattern—not live traffic or measured footfall.</p>
      ...
      <div className="mt-3 space-y-2">
        {selected.recommendedVenues.map(({ venue, contributions }) => <button key={venue.id} type="button" onClick={() => onSelectVenue(venue)} className="flex w-full items-center gap-3 rounded-2xl border border-white/50 bg-background/40 px-4 py-3 text-left backdrop-blur-md hover:border-primary/35 hover:bg-background/60">

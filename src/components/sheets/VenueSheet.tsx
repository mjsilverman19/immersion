...
import type { RankedVenue, TasteProfile, UserPlaceState } from "@/types/data";

export function VenueSheet({ ranked, tasteProfile, state, onUpdate, onDirections, onClose }: { ranked: RankedVenue | null; tasteProfile: TasteProfile | null; state: UserPlaceState | undefined; onUpdate: (patch: Partial<UserPlaceState>) => void; onDirections: () => void; onClose: () => void }) {
...
function Action({ active, label, icon, onClick }: { active: boolean; label: string; icon: React.ReactNode; onClick: () => void }) {

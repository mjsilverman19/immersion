          ...
          {(showAllComplements ? complements : complements.slice(0, 3)).map((result) => (
            <button key={result.venue.id} type="button" onClick={() => onSelectPlace(result.venue.id)} className="flex w-full items-center gap-3 rounded-2xl border border-white/50 bg-background/40 px-3 py-2.5 text-left hover:border-primary/35 hover:bg-background/60">
              <span className="min-w-0 flex-1">
                <span className="type-venue block truncate text-[17px]">{result.venue.name}</span>
...
function RetrievalSection({ title, subtitle, expanded, total, onExpandedChange, children }: { title: string; subtitle: string; expanded: boolean; total: number; onExpandedChange: (expanded: boolean) => void; children: React.ReactNode }) {
  return (
    <div>
      ...
      {total > 3 && <button type="button" onClick={() => onExpandedChange(!expanded)} className="type-button mt-2 text-primary underline underline-offset-4">{expanded ? "Show fewer" : `Show all ${total}`}</button>}
    </div>
  );
}

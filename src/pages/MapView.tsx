    ...
    <main className="relative h-[100dvh] w-full overflow-hidden bg-background">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-30 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] md:p-5">
        <div className="flex items-start justify-between gap-2">
          <div className="brand-surface pointer-events-auto rounded-2xl px-4 py-2.5"><h1 className="type-logo">immersion</h1><p className="type-subtitle mt-1 text-muted-foreground">New York City</p></div>
                  ...
                  {INTENT_ORDER.map((intentId) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn("type-button flex min-h-10 w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left transition hover:bg-background/70", selected && "bg-background/60")}
                      >
        ...
        {tasteProfile && <>
          <div className="pointer-events-auto mt-2 sm:hidden">
            <button
              ...
              className="brand-surface type-button flex min-h-10 items-center gap-2 rounded-full px-3 py-2"
            >
            ...
            {tasteControlsOpen && <div className="brand-surface mt-1.5 w-[min(88vw,330px)] rounded-2xl p-2.5">
              <div className="flex gap-1">
                <button type="button" onClick={() => { setMapMode("personalized"); setTasteControlsOpen(false); }} aria-pressed={mapMode === "personalized"} className={cn("type-button min-h-10 flex-1 rounded-full px-3 py-2", mapMode === "personalized" ? "bg-primary text-primary-foreground" : "hover:bg-background/60")}>Your map</button>
                <button type="button" onClick={() => { setMapMode("baseline"); setTasteControlsOpen(false); }} aria-pressed={mapMode === "baseline"} className={cn("type-button min-h-10 flex-1 rounded-full px-3 py-2", mapMode === "baseline" ? "bg-foreground text-background" : "hover:bg-background/60")}>City baseline</button>
              </div>
